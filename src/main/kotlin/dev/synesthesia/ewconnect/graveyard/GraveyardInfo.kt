package dev.synesthesia.ewconnect.graveyard

import dev.synesthesia.ewconnect.EwConnect
import dev.synesthesia.ewconnect.event.treasurehunt.TreasureHuntData
import dev.synesthesia.ewconnect.utils.Location
import io.netty.buffer.Unpooled
import net.minecraft.core.UUIDUtil
import net.minecraft.network.RegistryFriendlyByteBuf
import net.minecraft.network.codec.ByteBufCodecs
import net.minecraft.network.codec.StreamCodec
import org.mapdb.DataInput2
import org.mapdb.DataOutput2
import org.mapdb.Serializer
import java.util.UUID

data class GraveyardInfo(val uuid: UUID, val location: Location, val epitaph: String) {
    
    companion object {
        val STREAM_CODEC = StreamCodec.composite(
            UUIDUtil.STREAM_CODEC, GraveyardInfo::uuid,
            Location.STEAM_CODEC, GraveyardInfo::location,
            ByteBufCodecs.STRING_UTF8, GraveyardInfo::epitaph,
            ::GraveyardInfo
        )

        val DB_SERIALIZER = object : Serializer<GraveyardInfo> {

            override fun serialize(data: DataOutput2, graveyardInfo: GraveyardInfo) {
                val buffer = RegistryFriendlyByteBuf(Unpooled.buffer(), EwConnect.server.registryAccess())
                STREAM_CODEC.encode(buffer, graveyardInfo)

                val bytes = ByteArray(buffer.readableBytes())
                buffer.readBytes(bytes)

                data.writeInt(bytes.size)
                data.write(bytes)
            }

            override fun deserialize(data: DataInput2, available: Int): GraveyardInfo? {
                var size = data.readInt()
                var byteArray = ByteArray(size)

                data.readFully(byteArray)
                var buffer =
                    RegistryFriendlyByteBuf(Unpooled.wrappedBuffer(byteArray), EwConnect.server.registryAccess())

                return STREAM_CODEC.decode(buffer)
            }
        }
    }
    
}